...
str_res fromDouble(long double value) {
    ...
    str.len = floor(value == 0 ? 0 : log10(value)) + 8;
...
static inline str_res_id strGrow(str* string) {
        ...
        if (original_address != string->contents) return str_corrupted_pointer;
#endif
        return 0;
}
...
str_res_id appendStr(str *string, str *value) {
    if (value == NULL)
...
int insertAt_char(str *str, size_t index, char in) {
    if (str->len + 1 >= str->max_len)
        if (strGrow(str) != str_no_error) return -1;
    // shift all of the bytes to the right one to make room for the new byte that is being inserted
    memmove(str->contents + index, str->contents + index + 1, str->len - (index + 1) );
